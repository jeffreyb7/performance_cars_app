<script setup>
  ...
  const width = computed(() => {
    return props.data.model.length * 200
  });
  ...
  const x_positions = computed(() => {
    const x_scale = d3.scaleBand().domain(props.data.model).range([margin.left, width.value - margin.right]).padding(0.4);
    const bar_x = {};
  ...
  const vXaxis = {
    mounted(el) {
      d3.select(el).call(d3.axisBottom(d3.scaleBand().domain(props.data.model).range([margin.left, width.value - margin.right]).padding(0.4)))
    }
  }
